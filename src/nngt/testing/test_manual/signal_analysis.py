...
from scipy.signal import ( vectorstrength, spectrogram, fftconvolve, gaussian,
                           argrelmax, firwin, lfilter )
...
simtime = 500. # Simulation time in ms
...
N_rec     = 200      # record from N_rec neurons
...
nest.Connect(nodes_ex, nodes_ex, {'rule':'fixed_indegree', 'indegree':CE}, syn_spec="excitatory")
nest.Connect(nodes_ex, nodes_in, {'rule':'fixed_indegree', 'indegree':CE}, syn_spec="excitatory")
nest.Connect(nodes_in, nodes_ex, {'rule':'fixed_indegree', 'indegree':CI}, syn_spec="inhibitory")
nest.Connect(nodes_in, nodes_in, {'rule':'fixed_indegree', 'indegree':CI}, syn_spec="inhibitory")
...
sc = ax1.scatter(times0,senders0,c='b',s=7)
ax1.set_ylim(0,N_rec+1)
ax1.set_xlim(-0.01*simtime,1.01*simtime)
plt.title(stri)
plt.legend()
ax1.set_label('Neuron ID')

'bottom panel'
binsize = 1.0 #  double ,miliseconds, used in firing rate calculation
ax2 = fig.add_axes([0.1, 0.08, 0.85, 0.17], sharex=ax1)
histdata=plt.hist(times,bins=int(simtime)/binsize, visible=False)
bar_height = max(histdata[0][-200:]/(0.001*binsize*(N_rec)))
ax2.bar(histdata[1][:-1],histdata[0]/(0.001*binsize*(N_rec)),width=binsize)
...
timesteps = np.linspace(0., simtime, num_steps)
val = np.zeros(num_steps)
idx_times = np.digitize(times0, timesteps)
val[idx_times] += 1

w_local = gaussian(int(2./dt), 0.2/dt)
#~ w_local = gaussian(int(50./dt), 8./dt)
val = fftconvolve(val, w_local, 'same')

#~ w_global = blackman(num_steps)
#~ val *= w_global

fig_tmp, ax_tmp = plt.subplots()
ax_tmp.plot(timesteps,val)

fig2, ax3 = plt.subplots()
f, t, Sxx = spectrogram(val, 1000./dt)
...
lim = int(num_steps/2)
valf = fft(val)
xf = np.linspace(0., 1000./(2.*dt), lim)
fig3, ax4 = plt.subplots()
#~ ax4.semilogy(xf[1:lim], 2./lim * np.abs(valf[1:lim]), '-r')
ax4.plot(xf[1:lim], 1./lim * np.abs(valf[1:lim]), '-r')

''' vector strength '''
w_local = gaussian(int(30./dt), 5./dt)
valavg = fftconvolve(val, w_local, 'same')

fig4,(ax5,ax6) = plt.subplots(2,sharex=True)
ax5.plot(timesteps,valavg)
#~ ax6.plot(timesteps,val_fft)
idx_lmax = argrelmax(valavg)[0]
print(idx_lmax)
maxlim = np.max(valavg)/3.
lst_max = [ timesteps[i] for i in idx_lmax if valavg[i] > maxlim ]

periods = np.concatenate( ([lst_max[0]], np.diff(lst_max), [simtime-lst_max[-1]]) )
print(periods)

#~ period = 1000./xf[np.argmax(valf[1:int(num_steps/2)])]
print(vectorstrength(times0,periods), np.average(periods))
